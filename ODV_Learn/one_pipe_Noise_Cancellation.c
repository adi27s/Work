...
void app_main(void)
{
    ...
    audio_element_handle_t fatfs_stream_writer, i2s_stream_reader, audio_encoder,fatfs_stream_writer2,audio_encoder2;
    ...
    fatfs_stream_writer2 = fatfs_stream_init(&fatfs_cfg);
    ...
    audio_encoder2 = wav_encoder_init(&wav_cfg);
    ...
    /* AFE starts here*/
    ESP_LOGI(TAG, "[3.5] Create algorithm stream for aec");
    ...
    algo_config.algo_mask = ALGORITHM_STREAM_USE_NS;
    algo_config.task_stack = (10 * 1024);
    algo_config.mic_ch = 2;
    ...
    ESP_LOGI(TAG, "[3.6] Create audio encoder to handle data");
    // audio_encoder = wav_encoder_init(&wav_cfg);
    ESP_LOGI(TAG, "[3.7] Register all elements to audio pipeline");
    audio_pipeline_register(pipeline, audio_encoder2, "wav_2");
    audio_pipeline_register(pipeline, fatfs_stream_writer2, "file_2");
    /* AFE ends here*/

    ESP_LOGI(TAG, "[3.8] Link it together [codec_chip]-->i2s_stream-->audio_encoder-->fatfs_stream-->[sdcard]");
    const char *link_tag[6] = {"i2s", "wav", "file", "algo", "wav_2", "file_2"};
    // const char *link_tag[4] = {"i2s", "wav", "file", "algo"};
    audio_pipeline_link(pipeline, &link_tag[0], 6);
    ESP_LOGI(TAG, "[3.9] Set music info to fatfs");
    ...
    ESP_LOGI(TAG, "[3.10] Set up  uri");
    audio_element_set_uri(fatfs_stream_writer, "/sdcard/rec.wav");
    audio_element_set_uri(fatfs_stream_writer2, "/sdcard/rec_post_algo.wav");
    ...
    audio_pipeline_unregister(pipeline, audio_encoder2);
    audio_pipeline_unregister(pipeline, i2s_stream_reader);
    audio_pipeline_unregister(pipeline, fatfs_stream_writer);
    audio_pipeline_unregister(pipeline, fatfs_stream_writer2);
    ...
    audio_element_deinit(fatfs_stream_writer2);
    audio_element_deinit(i2s_stream_reader);
#if defined (CONFIG_CHOICE_OPUS_ENCODER) && defined (CONFIG_CHOICE_AAC_ENCODER) && defined (CONFIG_ESP_LYRAT_MINI_V1_1_BOARD)
    audio_element_deinit(resample);
#endif
    audio_element_deinit(audio_encoder);
    audio_element_deinit(audio_encoder2);
